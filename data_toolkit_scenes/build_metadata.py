...
def update_metadata(path, opt):
    ...
    if opt.from_merged_records:
        ...
        df_files = [
            f for f in os.listdir(os.path.join(path, "new_records")) if f.startswith("part_") and f.endswith(".csv")
        ]
    df_parts = []
    for f in df_files:
        try:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--render_cond_root", type=str, default=None, help="Directory to save the render condition files"
    )
    ...
    parser.add_argument(
        "--ss_latent_root", type=str, default=None, help="Directory to save the sparse structure latent files"
    )
    ...
    parser.add_argument(
        "--rebuild", action="store_true", help="Rebuild metadata from scratch, ignore existing metadata."
    )
