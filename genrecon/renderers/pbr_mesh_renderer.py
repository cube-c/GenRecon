...
class PbrMeshRenderer:
    ...
    def render(
        ...
        if chunk_size is None:
            out_dict, shaded, depth, normal, alpha = self._render_peel_layers(
                mesh,
                vertices,
                vertices_orig,
                vertices_camera,
            ...
            out_dict, shaded, depth, normal, alpha = self._render_chunked_layer0(
                mesh,
                vertices,
                vertices_orig,
                vertices_camera,
    ...
    def _render_peel_layers(
        ...
        """Depth-peeled rendering path (original behavior). Composites `peel_layers`
        layers front-to-back so semi-transparent surfaces blend correctly.
        """
        if "dr" not in globals():
            import nvdiffrast.torch as dr
        ...
        out_dict = edict()
        shaded = torch.zeros(
            (num_envmaps, resolution * ssaa, resolution * ssaa, 3), dtype=torch.float32, device=self.device
        )
        ...
        with dr.DepthPeeler(self.glctx, vertices_clip, faces, (resolution * ssaa, resolution * ssaa)) as peeler:
            for _ in range(self.rendering_options["peel_layers"]):
                ...
                gb_normal = torch.where(
                    torch.sum(gb_normal * (pos - rays_o), dim=-1, keepdim=True) > 0, -gb_normal, gb_normal
                )
        ...
        return out_dict, shaded, depth, normal, alpha

    def _render_chunked_layer0(
        self,
        mesh,
        vertices,
        vertices_orig,
        ...
        """Chunked single-layer rasterization. Splits faces into groups of
        `chunk_size` to keep each dr.rasterize call below nvdiffrast's per-call
        triangle limits, then Z-buffers the chunks to obtain the globally nearest
        surface per pixel and shades that merged layer. Equivalent to peel_layers=1,
        so transparency compositing across layers is lost — use only when the mesh
        ...
        pos_buf = torch.zeros((H, W, 3), dtype=torch.float32, device=device)
        depth_buf = torch.full((H, W, 1), 1e10, dtype=torch.float32, device=device)
        normal_buf = torch.zeros((H, W, 3), dtype=torch.float32, device=device)
        cam_normal_buf = torch.zeros((H, W, 3), dtype=torch.float32, device=device)
        mask_buf = torch.zeros((H, W, 1), dtype=torch.float32, device=device)
        basecolor_buf = torch.zeros((H, W, 3), dtype=torch.float32, device=device)
        metallic_buf = torch.zeros((H, W, 1), dtype=torch.float32, device=device)
        roughness_buf = torch.zeros((H, W, 1), dtype=torch.float32, device=device)
        alpha_buf = torch.zeros((H, W, 1), dtype=torch.float32, device=device)
        z_buffer = torch.full((1, H, W), torch.inf, dtype=torch.float32, device=device)

        for i_f in range(0, faces.shape[0], chunk_size):
            faces_chunk = faces[i_f : i_f + chunk_size]
            rast, rast_db = dr.rasterize(self.glctx, vertices_clip, faces_chunk, (H, W))
            ...
            pos = dr.interpolate(vertices, rast, faces_chunk)[0][0]
            gb_depth = dr.interpolate(vertices_camera[..., 2:3].contiguous(), rast, faces_chunk)[0][0]

            face_normal_chunk = face_normal[i_f : i_f + chunk_size]
            gb_normal = dr.interpolate(
                face_normal_chunk.unsqueeze(0),
                rast,
                torch.arange(face_normal_chunk.shape[0], dtype=torch.int, device=device)
                .unsqueeze(1)
                .repeat(1, 3)
                .contiguous(),
            )[0][0]
            ...
            if isinstance(mesh, MeshWithVoxel):
                if "grid_sample_3d" not in globals():
                    from flex_gemm.ops.grid_sample import grid_sample_3d
                attr_mask = rast[..., -1:] > 0
                xyz = dr.interpolate(vertices_orig, rast, faces_chunk)[0]
                xyz = ((xyz - mesh.origin) / mesh.voxel_size).reshape(1, -1, 3)
                img = grid_sample_3d(
                    mesh.attrs,
                    torch.cat([torch.zeros_like(mesh.coords[..., :1]), mesh.coords], dim=-1),
                ...
                img = img.reshape(1, H, W, mesh.attrs.shape[-1]) * attr_mask
                gb_basecolor = img[0, ..., mesh.layout["base_color"]]
                gb_metallic = img[0, ..., mesh.layout["metallic"]]
                gb_roughness = img[0, ..., mesh.layout["roughness"]]
                gb_alpha = img[0, ..., mesh.layout["alpha"]]
                ...
                tri_id = rast[0, :, :, -1:]
                attr_mask = tri_id > 0
                uv_coords_chunk = mesh.uv_coords[i_f : i_f + chunk_size].reshape(1, -1, 2)
                texc, texd = dr.interpolate(
                    uv_coords_chunk,
                    rast,
                    torch.arange(uv_coords_chunk.shape[1], dtype=torch.int, device=device).reshape(-1, 3),
                    rast_db=rast_db,
                    diff_attrs="all",
                )
                texc = torch.nan_to_num(texc, nan=0.0, posinf=1e3, neginf=-1e3)
                ...
                texd = torch.nan_to_num(texd, nan=0.0, posinf=1e3, neginf=-1e3)
                texd = torch.clamp(texd, min=-1e3, max=1e3)
                # tri_id is chunk-local; offset by i_f to index the global material_ids.
                global_face_id = (tri_id - 1 + i_f).clamp(min=0, max=mesh.material_ids.shape[0] - 1).long()
                mid = mesh.material_ids[global_face_id]
                gb_basecolor = torch.zeros((H, W, 3), dtype=torch.float32, device=device)
                gb_metallic = torch.zeros((H, W, 1), dtype=torch.float32, device=device)
                gb_roughness = torch.zeros((H, W, 1), dtype=torch.float32, device=device)
                gb_alpha = torch.zeros((H, W, 1), dtype=torch.float32, device=device)
                for id, mat in enumerate(mesh.materials):
                    mat_mask = (mid == id).float() * attr_mask.float()
                    mat_texc = texc * mat_mask
                    mat_texd = texd * mat_mask
            ...
            pos_buf[zf] = pos[zf]
            depth_buf[zf] = gb_depth[zf]
            normal_buf[zf] = gb_normal[zf]
            cam_normal_buf[zf] = gb_cam_normal[zf]
            mask_buf[zf] = 1.0
            basecolor_buf[zf] = gb_basecolor[zf]
            metallic_buf[zf] = gb_metallic[zf]
            roughness_buf[zf] = gb_roughness[zf]
            alpha_buf[zf] = gb_alpha[zf]

        out_dict = edict()
        out_dict.normal = -cam_normal_buf * 0.5 + 0.5
        out_dict.mask = mask_buf
        out_dict.base_color = basecolor_buf
        out_dict.metallic = metallic_buf
        out_dict.roughness = roughness_buf
        out_dict.alpha = alpha_buf

        gb_bc = torch.clamp(basecolor_buf, 0.0, 1.0) ** 2.2
        gb_m = torch.clamp(metallic_buf, 0.0, 1.0)
        gb_r = torch.clamp(roughness_buf, 0.0, 1.0)
        gb_a = torch.clamp(alpha_buf, 0.0, 1.0)
        gb_orm = torch.cat([torch.zeros_like(gb_m), gb_r, gb_m], dim=-1)
        gb_shaded = torch.stack(
            [
                e.shade(
                    pos_buf.unsqueeze(0),
                    normal_buf.unsqueeze(0),
                    gb_bc.unsqueeze(0),
                    gb_orm.unsqueeze(0),
                    rays_o,
                    specular=True,
        ...
        # Single-layer compositing: equivalent to the peel loop's first iteration.
        w = gb_a
        shaded = w * gb_shaded
        depth = depth_buf
        normal = cam_normal_buf
        alpha = w
