...
class SparseStructureLatentVisMixin:
    def __init__(
        self,
        *args,
        pretrained_ss_dec: str = "microsoft/TRELLIS-image-large/ckpts/ss_dec_conv3d_16l8_fp16.json",
...
class SparseStructureLatent(SparseStructureLatentVisMixin, StandardDatasetBase):
    ...
    @staticmethod
    def _collate_group(sub_batch):
        """Collate a list of samples into a single pack, handling flat-format view tensors."""
        pack = {}
        keys = [k for k in sub_batch[0].keys()]
        for k in keys:
            if isinstance(sub_batch[0][k], torch.Tensor):
                tensors = [b[k] for b in sub_batch]
                shapes = [t.shape for t in tensors]
                if all(s == shapes[0] for s in shapes):
                    pack[k] = torch.stack(tensors)
                else:
                    # variable first dim — flat-format view tensors (cond, extrinsics, intrinsics)
                    pack[k] = torch.cat(tensors, dim=0)
                    if "cond_batch_idx" not in pack:
                        pack["cond_batch_idx"] = torch.cat(
                            [torch.full((t.shape[0],), i, dtype=torch.long) for i, t in enumerate(tensors)]
                        )
    ...
    def collate_fn(batch, split_size=None):
        if split_size is None:
            return SparseStructureLatent._collate_group(batch)
        # Split into split_size micro-batches with round-robin assignment so that
        # each group has approximately equal size (x_0 is a fixed-size dense tensor).
        groups = [batch[i::split_size] for i in range(split_size) if batch[i::split_size]]
        return [SparseStructureLatent._collate_group(g) for g in groups]


class RoomCameraConditionedSparseStructureLatent(RoomCameraConditionedMixin, SparseStructureLatent):
    """
