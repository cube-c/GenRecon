...
from ..renderers import EnvMap, PbrMeshRenderer
...
from .components import RoomCameraConditionedMixin, StandardDatasetBase
...
class SLatPbrVisMixin:
    ...
    def visualize_sample(self, sample: dict):
        ...
        renderer.rendering_options.chunk_size = 5_000_000
        envmap = EnvMap(
            torch.tensor(
...
class SLatPbr(SLatPbrVisMixin, StandardDatasetBase):
    ...
    def filter_metadata(self, metadata):
        ...
        metadata = metadata[metadata["pbr_latent_tokens"] >= self.min_tokens]
        stats[f"Num tokens >= {self.min_tokens}"] = len(metadata)
        if self.full_pbr:
            ...
            for k in keys:
                if isinstance(sub_batch[0][k], torch.Tensor):
                    ...
                    if all(s == shapes[0] for s in shapes):
                        ...
                        # variable first dim (e.g. different number of views per sample)
...
class RoomCameraConditionedSLatPbr(RoomCameraConditionedMixin, SLatPbr):
    """
    Image conditioned structured latent for pbr generation
    """
