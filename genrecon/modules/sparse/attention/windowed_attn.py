...
def calc_window_partition(
    ...
    shifted_indices = (shifted_coords * torch.tensor(OFFSET, device=tensor.device, dtype=torch.int32).unsqueeze(0)).sum(
        dim=1
    )
    ...
    if config.ATTN == "xformers":
        ...
        attn_func_args = {
            "cu_seqlens": torch.cat(
                [torch.tensor([0], device=tensor.device), torch.cumsum(seq_lens, dim=0)], dim=0
            ).int(),
...
def sparse_windowed_scaled_dot_product_self_attention(
    ...
    if serialization_spatial_cache is None:
        ...
        qkv.register_spatial_cache(
            serialization_spatial_cache_name, (fwd_indices, bwd_indices, seq_lens, attn_func_args)
        )
    else:
...
def sparse_windowed_scaled_dot_product_cross_attention(
    ...
    if q_serialization_spatial_cache is None:
        q_fwd_indices, q_bwd_indices, q_seq_lens, q_attn_func_args = calc_window_partition(
            q, q_window_size, q_shift_window
        )
