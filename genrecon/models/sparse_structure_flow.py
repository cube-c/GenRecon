...
from ..modules.cond_3D.aggregation_net import MultiViewFeatAggregator
from ..modules.cond_3D.projection import Projection
...
class SparseStructureFlowModel(nn.Module):
    ...
    def add_projections(
        ...
        self.aggregator = MultiViewFeatAggregator(
    ...
    def get_3D_cond(self, img_feats_all, extrinsics, intrinsics, x_0):
        if not hasattr(self, "projection") or not hasattr(self, "aggregator"):
            return None
        projection, mask, camera_emb = self.projection(
            img_feats_all, extrinsics, intrinsics
        )  # [B, n_views, num_voxels, D_cond]
        projection_aggr = self.aggregator(projection, mask, camera_emb)
    ...
    def forward(
        ...
        x: torch.Tensor,
        t: torch.Tensor,
        cond: dict,  # {"cond_2D": [B, T, D], "cond_3D": [B, R^3, D]}
    ) -> torch.Tensor:
        assert [*x.shape] == [
            x.shape[0],
            self.in_channels,
        ...
        if cond_3D is not None:
            cond_3D = manual_cast(cond_3D, self.dtype)  # [B, R^3, D]

        assert cond_2D.ndim == 3, f"expected cond_2D to have 3 dimensions but got {cond_2D.ndim}"
        assert (
        ...
        ), f"expected condition 3D  batch size {x.shape[0]} but got {cond_3D.shape[0]}"
        assert (
            cond_3D is None or cond_3D.shape[1] == self.resolution**3
        ), f"expected condition 3D with {self.resolution ** 3} voxels but got {cond_3D.shape[1]}"

        for i, block in enumerate(self.blocks):
            if self.proj_linears is not None:
                h += self.proj_linears[i](cond_3D)
