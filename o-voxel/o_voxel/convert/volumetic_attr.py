    ...
    for sid, (name, g) in tqdm(
        ...
        if verbose:
            ...
            if g.visual.material.emissiveTexture is not None:
                print(
                    f"  Emissive texture: {g.visual.material.emissiveTexture.size} {g.visual.material.emissiveTexture.mode}"
                )
            ...
            if g.visual.material.normalTexture is not None:
                print(
                    f"  Normal texture: {g.visual.material.normalTexture.size} {g.visual.material.normalTexture.mode}"
                )
        ...
        alphaMode = (
            ALPHA_MODE_ENUM[g.visual.material.alphaMode] if g.visual.material.alphaMode in ALPHA_MODE_ENUM else 0
        )
        alphaCutoff = g.visual.material.alphaCutoff if g.visual.material.alphaCutoff is not None else 0.5
        alphaFactor = (
            g.visual.material.baseColorFactor[3] / 255 if g.visual.material.baseColorFactor is not None else 1.0
        )
        alphaTexture = (
            torch.tensor(np.array(g.visual.material.baseColorTexture.convert("RGBA"))[..., 3], dtype=torch.uint8)
...
def blender_dump_to_volumetric_attr(
    ...
    for object in dump["objects"]:
        triangles = torch.tensor(object["vertices"][object["faces"]], dtype=torch.float32).reshape(-1, 3, 3) - aabb[
            0
        ].reshape(1, 1, 3)
