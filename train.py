...
from genrecon import datasets, trainers
from genrecon.utils.dist_utils import setup_dist
from genrecon.utils.model_wrapper_utils import build_single_model
...
def main(local_rank, cfg):
    ...
    # Load data
    train_dataset = getattr(datasets, cfg.dataset.name)(cfg.data_dir, **cfg.dataset.args)
    val_dataset = None
    if cfg.val_data_dir is not None:
        val_dataset = getattr(datasets, cfg.dataset.name)(cfg.val_data_dir, **cfg.dataset.args)

    # Build model
    model_dict = {name: build_single_model(model_cfg).cuda() for name, model_cfg in cfg.models.items()}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_steps", type=int, default=None, help="Override trainer.args.max_steps from the config")
    ## dubug
    ...
    if opt.max_steps is not None:
        print(f"[override] trainer.args.max_steps: {config['trainer']['args'].get('max_steps')} -> {opt.max_steps}")
        config["trainer"]["args"]["max_steps"] = opt.max_steps
